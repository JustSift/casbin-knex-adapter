...
class KnexAdapter {
  ...
  async loadPolicy(model) {
    const policies = await this._getKnex().select();
  ...
  async addPolicy(sec, ...args) {
    ...
    await this._getKnex().insert(policy);
  }

  async removePolicy(sec, ...args) {
    const policy = this.createPolicy(...args);
    await this._getKnex().delete().where(policy);
  }

  async removeFilteredPolicy(sec, ...args) {
    const filteredPolicy = this.createFilteredPolicy(...args);
    await this._getKnex().delete().where(filteredPolicy);
  }
}
