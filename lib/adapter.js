...
class KnexAdapter {
  ...
  async loadPolicy(model) {
    const policies = await this.knex(this.tableName).select();
  ...
  async addPolicy(sec, ...args) {
    ...
    await this.knex(this.tableName).insert(policy);
  }

  async removePolicy(sec, ...args) {
    const policy = this.createPolicy(...args);
    await this.knex(this.tableName).delete().where(policy);
  }

  async removeFilteredPolicy(sec, ...args) {
    const filteredPolicy = this.createFilteredPolicy(...args);
    await this.knex(this.tableName).delete().where(filteredPolicy);
  }
}
